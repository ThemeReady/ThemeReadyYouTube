...
vec4 getColor(in vec2 texCrd) {
  ...
  if (uLutSize > 0.0)
  {
    ...
    float halfPixel = 1.0 / (uLutSize * uLutSize);
    // Reduce the range of R by 1 pixel so we can look up centre to centre.
    rgb.r *= (uLutSize - 1.0) / uLutSize;
    // Since G is the discontinuous color, take 2 snapshots and blend them
    // together.
    vec2 UV1 = vec2(halfPixel + rgb.r / uLutSize
        + floor(rgb.g * (uLutSize - 1.0)) / uLutSize, rgb.b);
    vec2 UV2 = vec2(halfPixel + rgb.r / uLutSize
        + ceil(rgb.g * (uLutSize - 1.0)) / uLutSize, rgb.b);
    float blend = fract(rgb.g * (uLutSize - 1.0));
